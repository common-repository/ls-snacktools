...
function ls_snack_i18n_init() {
    ...
    load_plugin_textdomain('ls_snack', false, plugin_basename(dirname(__FILE__)) . '/languages/');
}
...
function wp_embed_handler_ls_snack( $matches, $attr, $url, $rawattr ) {
    ...
    $embed = '<div class="fitvid" align="center">'
...
wp_embed_register_handler( 'podsnack', '/http:\/\/(www.podsnack.com)\/(\w+)/i', 'wp_embed_handler_ls_podsnack' );

function wp_embed_handler_ls_podsnack($matches, $attr, $url, $rawattr) {
     $args = wp_embed_defaults();
    $hash = ls_after_last('=', $url);
    if (is_bool($hash)) {
        $hash = ls_after_last('/', $url);
    ...
    $embed = '<div class="fitvid" align="center">'
            . '<iframe src="http://files.podsnack.com/iframe/embed.html?hash=' . $hash . '&amp;'
            . 'wmode=transparent" seamless="seamless" scrolling="no" frameborder="0" allowtransparency="true"  style="width: ' . ($width - 5) . 'px; height: 265px;"></iframe>'
            . '	<br/><a href="' . $url . '">' . __('From podsnack.com', 'ls_snack') . '</a></div>';

    return apply_filters('embed_ls_podsnack', $embed, $matches, $attr, $url, $rawattr);
}
